...
Raphael.fn.g.linechart = function (x, y, width, height, valuesx, valuesy, opts) {
    ...
    for (i = 0, ii = valuesy.length; i < ii; i++) {
        ...
        for(var xx=0;xx<=20;xx++)
            no_of_wickets[i][xx]=0;

        for(xx=0;xx<opts["wickets"][i].length;xx++)
            no_of_wickets[i][opts["wickets"][i][xx]++]++;
        ...
        for (var j = 0, jj = valuesy[i].length; j < jj; j++) {
            ...
            for (xx = 0; xx < no_of_wickets[i][j]; xx++) {
                var other_team_clash=0;
                if(i==1 && valuesy[i][j] == valuesy[i-1][j])
                    other_team_clash=no_of_wickets[i-1][j]*12;
                X = x + gutter + ((valuesx[i] || valuesx[0])[j] - minx) * kx,
                Y = y + height - gutter - (valuesy[i][j] - miny) * ky+(xx*12)+other_team_clash;
                symset.push(this.g[Raphael.fn.g.markers["o"]](X, Y, (opts.width || 2) * 3).attr({fill: colors[i], stroke: "none"}));
            }
        }
        if (opts.smooth) {
        ...
        if (opts.shade) {
            ...
            //var custom_circles=opts.wickets[i];
        //var counter=0;
    ...
    function createDots(f) {
        ...
        for (var i = 0, ii = valuesy.length; i < ii; i++) {            
            ...
            for (var j = 0, jj = valuesy[i].length; j < jj; j++) {
                ...
                C.message=valuesy[i][j]+" RR "+" ("+opts["team_names"][i]+")";
                if(i==1 && C.value == valuesy[i-1][j])
                    C.message+=" "+no_of_wickets[i-1][j]+" Wickets ("+opts["team_names"][i-1]+")";
                
                C.angle=opts["angles"][i];
                f && f.call(C);
            }
        }
        !f && (dots = cvrs);
