...
class DJBase {
	...
	const 	 STDOUT = 0;
	const 	 FILE = 1;
	const    LOGFILENAME = "djjob.log";


    private static $log_level = self::DEBUG;

    private static $log_type = self::STDOUT;

    private static $log_file = self::LOGFILENAME;
    ...
    public static function setLogType($const) {
        self::$log_type = $const;
    }

	public static function setLogFile($file_path) {
    ...
    protected static function log($mesg, $severity=self::CRITICAL) {
        if ($severity >= self::$log_level) {
			if(self::$log_type == self::STDOUT)
            	printf("[%s] %s\n", date('Y-m-d H:i:s'), $mesg);
			else if (self::$log_type == self::FILE) {
...
class DJWorker extends DJBase {
    ...
    public function handleSignal($signo) {
        $signals = array(
            SIGTERM => "SIGTERM", // generated by kill
            SIGINT  => "SIGINT"  // generated by cntrl+c
        );
    ...
    public function getNewJob() {
	 ...
	 $rs = $this->runQuery("
            ...
            AND    attempts < ?
            ORDER BY run_at ASC, priority DESC, created_at DESC
            LIMIT  10
...
//	    Yii::app()->getDb()->setActive(true);
    ...
    public static function enqueue($handler, $queue = "default", $run_at = null, $priority=0) {
        $affected = self::runUpdate(
            "INSERT INTO jobs (handler, queue, run_at, priority, created_at) VALUES(?, ?, ?, ?, NOW())",
            array(serialize($handler), (string) $queue, $run_at, $priority)
        );
	...
	public static function getNextJob($queue="default") {
		$rs = self::runQuery("
            SELECT id
            FROM   jobs
            WHERE  queue = ?
        ...
        ", array($queue));
		if(count($rs) > 0 ) {
			$job = new DJJob("Bulk", $rs[0]["id"], array(
	        "max_attempts" => 5,
	    	));
			return $job;
		}
    ...
    public static function runAll($queue = "default") {
		while(true) {
			$job = self::getNextJob($queue);
			if($job) {
	        	if ($job->acquireLock()) {
					$job->run();
					$job->releaseLock();
	...
	public static function runNext($queue = "default") {
		$job = self::getNextJob($queue);
		if($job) {
        	if ($job->acquireLock()) {
				$job->run();
